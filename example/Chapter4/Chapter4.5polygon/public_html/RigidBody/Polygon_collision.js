 ...
 * @memberOf Polygon
 * @param {Polygon} otherRect  another Polygon that being tested
...
/**
 * Check for collision between RigidPolygon and RigidPolygon
 * @param {Polygon} r1 Polygon object to check for collision status
 * @param {Polygon} r2 Polygon object to check for collision status against
...
Polygon.prototype.collidedRectRect = function (r1, r2, collisionInfo) {
    ...
    //find Axis of Separation for both Polygon
    status1 = r1.findAxisLeastPenetration(r2, collisionInfoR1);

    if (status1) {
        status2 = r2.findAxisLeastPenetration(r1, collisionInfoR2);
        if (status2) {
            //if both of Polygons are overlapping, choose the shorter normal as the normal       
...
Polygon.prototype.collidedRectCirc = function (otherCir, collisionInfo) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (projection > 0) {
            //if the center of circle is outside of Polygon
    ...
    if (!inside) {
        //the center of circle is outside of Polygon
        ...
        //the center of circle is inside of Polygon
        radiusVec = this.mFaceNormal[nearestEdge].scale(otherCir.mRadius);
        collisionInfo.setInfo(otherCir.mRadius - bestDistance, this.mFaceNormal[nearestEdge], circ2Pos.subtract(radiusVec));
    }
    return true;
